        ...
        call.enqueue(object : Callback<Produto> {
            override fun onResponse(call: Call<Produto>, response: Response<Produto>) {
                if (response.isSuccessful) {
                    ...
                    //  <<<<<<<<<<<<<<<<<<   Imagem  >>>>>>>>>>>>>>>>>>
                    obterImagemDoProduto(produtoId)
...
//  <<<<<<<<<<<<<<<<<<   Botão Deletar Produto  >>>>>>>>>>>>>>>>>>
    ...
    private fun obterImagemDoProduto(produtoId: Int) {
        apiService.obterImagemPorIdDoProduto(produtoId).enqueue(object : Callback<ResponseBody> {
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                if (response.isSuccessful) {
                    val imagemResponseBody: ResponseBody? = response.body()

                    if (imagemResponseBody != null) {
                        // Atualizar a ImageView com a imagem obtida
                        exibirImagemNaView(imagemResponseBody)
                    }
                } else {
                    // Trate o caso em que a resposta não foi bem-sucedida
                    Toast.makeText(applicationContext, "Adicione uma imagem ao seu produto", Toast.LENGTH_SHORT).show()
                }
            }
    ...
    private fun exibirImagemNaView(imagemResponseBody: ResponseBody) {
        try {
            val bitmap = BitmapFactory.decodeStream(imagemResponseBody.byteStream())
            val imageView1: ImageView? = findViewById(R.id.imagem1)
...
class ExibicaoProdutoActivity : AppCompatActivity() {
    ...
    private val imageList: MutableMap<Int, Bitmap> = mutableMapOf()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = ProductAdapter(productList, imageList) { productId ->
        ...
        apiService.obterProdutoPorId(productId).enqueue(object : Callback<Produto> {
            override fun onResponse(call: Call<Produto>, response: Response<Produto>) {
                if (response.isSuccessful) {
                    ...
                    if (produto != null) {
                        // Exiba o produto no layout
                        exibirProdutoNoLayout(produto)
                        obterImagemDoProduto(productId)
                    } else {
    ...
    private fun obterImagemDoProduto(productId: Int) {
        apiService.obterImagemPorIdDoProduto(productId).enqueue(object : Callback<ResponseBody> {
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                if (response.isSuccessful) {
                    val imagem = BitmapFactory.decodeStream(response.body()?.byteStream())

                    // Adicione a imagem ao mapa
                    val updatedImageList = imageList.toMutableMap()
                    updatedImageList[productId] = imagem

                    // Atualize o adaptador com o novo mapa de imagens
                    adapter.updateImageList(updatedImageList)
                } else {
                    //exibirToast("Erro ao obter a imagem. Código: ${response.code()}")
                }
            }

            override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                exibirToast("Falha na comunicação para obter a imagem. Mensagem: ${t.message}")
            }
        })
    }
